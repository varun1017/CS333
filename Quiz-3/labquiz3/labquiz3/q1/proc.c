  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        *first_scheduled_time = p->first_scheduled_time;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    scheduled_proc->first_scheduled_time = ticks;
...
void getSchedulerStats(int pid){
  struct proc *p;
  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      cprintf("Scheduler stats for process with pid %d\n", pid);
      cprintf("Arrival Time: %d\n", p->arrival_time);
      cprintf("First Scheduled Time: %d\n", p->first_scheduled_time);
      cprintf("Exit Time: %d\n", p->exit_time);
      release(&ptable.lock);
      return;
    }
