...
int alloc_inode(int mount_point){
	...
	*/
	...
	if(super_block.used_inodes == MAX_INODES){
		return -1;
	}
	int x = -1;
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	readblock(mounts[mount_point].device_fd,1+(inodenum/16),(char*)&metadata);
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	writeblock(mounts[mount_point].device_fd,1+(inodenum/16),(char*)&metadata);
...
int alloc_datablock(int mount_point){
	...
	*/
	struct superblock_t super_block;
	read_superblock(mount_point,&super_block);
	if(super_block.used_blocks == MAX_BLOCKS){
		return -1;
	}
	int x = -1;
	for(int i=3; i<MAX_BLOCKS; i++){
		if(super_block.block_bitmap[i] == 0){
			x = i;
			super_block.block_bitmap[i] = 1;
...
void free_datablock(int mount_point, int blocknum){
	...
	*/
    struct superblock_t super_block;
	read_superblock(mount_point,&super_block);
	super_block.block_bitmap[blocknum]=0;
	super_block.used_blocks--;
	write_superblock(mount_point,&super_block);
