...
int alloc_inode(int mount_point){
	...
	*/
	...
	read_superblock(mount_point,&super_block);
	int x = -1;
	for(int i=0; i<MAX_INODES; i++){
		if(super_block.inode_bitmap[i] == 0){
			x = i;
			super_block.inode_bitmap[i] = 1;
			super_block.used_inodes+=1;
			write_superblock(mount_point,&super_block);
			return i;
		}
	}
	return -1;
...
void free_inode(int mount_point, int inodenum){
	...
	*/
	struct superblock_t super_block;
	read_superblock(mount_point,&super_block);
    super_block.inode_bitmap[inodenum] = (char)0;
	super_block.used_inodes-=1;
    write_superblock(mount_point,&super_block);
}
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	if(inodenum<16){
		read_datablock(mount_point,1,(char*)&metadata);
	}
	else{
		read_datablock(mount_point,2,(char*)&metadata);
	}
	decrypt(mounts[mount_point].key,(char*)&metadata,BLOCKSIZE);
	*inodeptr = metadata.inodes[inodenum%16];
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	if(inodenum<16){
		read_datablock(mount_point,1,(char*)&metadata);
		decrypt(mounts[mount_point].key,(char*)&metadata,BLOCKSIZE);
		metadata.inodes[inodenum%16]=*inodeptr;
		encrypt(mounts[mount_point].key,(char*)&metadata,BLOCKSIZE);
		write_datablock(mount_point,1,(char*)&metadata);
	}
	else{
		read_datablock(mount_point,2,(char*)&metadata);
		decrypt(mounts[mount_point].key,(char*)&metadata,BLOCKSIZE);
		metadata.inodes[inodenum%16]=*inodeptr;
		encrypt(mounts[mount_point].key,(char*)&metadata,BLOCKSIZE);
		write_datablock(mount_point,2,(char*)&metadata);
	}
...
int alloc_datablock(int mount_point){
	...
	*/
	...
	for(int i=3; i<MAX_BLOCKS; i++){
		if(super_block.block_bitmap[i] == (char)0){
			x = i;
			super_block.block_bitmap[i] = (char)1;
			super_block.used_blocks+=1;
			write_superblock(mount_point,&super_block);
			return i;
		}
	}
...
void free_datablock(int mount_point, int blocknum){
	...
	*/
    struct superblock_t super_block;
	read_superblock(mount_point,&super_block);
	super_block.block_bitmap[blocknum]=(char)0;
	super_block.used_blocks-=1;
	write_superblock(mount_point,&super_block);
