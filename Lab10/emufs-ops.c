...
int emufs_delete(int dir_handle, char* path){
    ...
    */
    ...
    struct inode_t node;
    read_inode(dir[dir_handle].mount_point,y, &node);
    int j=0;
    for(int i=0;i<MAX_FILE_SIZE;i++){
        if(node.mappings[i] == x){
            j=i;
            break;
        }
    }
    for(int i=j+1;i<4;i++){
        node.mappings[i-1] = node.mappings[i]; 
    }
    node.size -= 1;
    write_inode(dir[dir_handle].mount_point,y, &node);
    return 1;
    ...
    */
    ...
    for(int i=0;i<4;i++){
        ...
        if(chinodnum>0){
            ...
            read_inode(dir[dir_handle].mount_point, chinodnum, &inode_ch);
    ...
    strcpy(inode2.name,name);
    inode2.type=type;
    inode2.parent=dir[dir_handle].inode_number;
    inode2.size=0;
    for(int i=0;i<4;i++){
        inode2.mappings[i]=-1;
    }
    write_inode(dir[dir_handle].mount_point, inode.mappings[x], &inode2);
...
int emufs_read(int file_handle, char* buf, int size){
    ...
    */
	...
	if (file.offset + size > MAX_FILE_SIZE * BLOCKSIZE) 
		return -1;
	...
	for (int j = 0; j < MAX_FILE_SIZE; j++){
		if(off < (j + 1) * BLOCKSIZE) {
			if (inode->mappings[j] != -1 && bytes < size){
				read_datablock(file.mount_point,inode->mappings[j],temp);

				for(int i = 0; i < BLOCKSIZE; i++) {
					if(i >= off - j * BLOCKSIZE && bytes < size) {
						buf[bytes] = temp[i];
						bytes++;
					}
				}	
	...
	for (int j = 0; j < MAX_FILE_SIZE; j++){
		if(off < (j + 1) * BLOCKSIZE) {
			if (inode->mappings[j] != -1 && bytes < size){
				...
				off = (j + 1)*BLOCKSIZE;
                write_datablock(file.mount_point,inode->mappings[j],temp);	
			    write_inode(file.mount_point,file.inode_number, inode);
			}
		}
        else if (inode->mappings[j] == -1){
			...
			int blocknum = alloc_datablock(file.mount_point);
			if(blocknum == -1) 
				return -1;
			
			inode->mappings[j] = blocknum;
			write_datablock(file.mount_point,inode->mappings[j], temp);
			write_inode(file.mount_point,file.inode_number, inode);
		}
			if(bytes == size) break;
