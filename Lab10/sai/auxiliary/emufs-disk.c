...
int alloc_inode(int mount_point){
	...
	*/
    struct superblock_t my_device;
	//will copy info into this struct::
	int found=-1;
	read_superblock(mount_point,&my_device);
	for(int i=0;i<MAX_INODES;i++){
       if(my_device.inode_bitmap[i]==0){
		 //found one::
		 found=i;
		 my_device.inode_bitmap[i]=1;
		 my_device.used_inodes+=1;
		 break;
	   }
	}
	if(found==-1){
	...
	write_superblock(mount_point,&my_device);
	return found;
...
void free_inode(int mount_point, int inodenum){
	...
	*/
	...
	read_superblock(mount_point,&my_device);
	my_device.inode_bitmap[inodenum]=(char)0;
	my_device.used_inodes-=1;
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
    struct metadata_t inode_block;
	if(inodenum>=16){
		read_datablock(mount_point,2,(char*)&inode_block);
	}
	else{
		read_datablock(mount_point,1,(char*)&inode_block);
	}
	decrypt(mounts[mount_point].key,(char*)&inode_block,BLOCKSIZE);
	*inodeptr=inode_block.inodes[inodenum%16];
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
   ...
   struct metadata_t inode_block;
	if(inodenum>=16){
		read_datablock(mount_point,2,(char*)&inode_block);
		decrypt(mounts[mount_point].key,(char*)&inode_block,BLOCKSIZE);
		inode_block.inodes[inodenum%16]=*inodeptr;
		encrypt(mounts[mount_point].key,(char*)&inode_block,BLOCKSIZE);
		write_datablock(mount_point,2,(char*)&inode_block);
	}
	else{
		read_datablock(mount_point,1,(char*)&inode_block);
		decrypt(mounts[mount_point].key,(char*)&inode_block,BLOCKSIZE);
		//printf("%d,%d,%d,%d\n",inodeptr->size,inodeptr->mappings[1],inode_block.inodes[inodenum%16].size,inode_block.inodes[inodenum%16].mappings[1]);
		inode_block.inodes[inodenum%16]=*inodeptr;
		//printf("%d,%d,%d,%d\n",inodeptr->size,inodeptr->mappings[1],inode_block.inodes[inodenum%16].size,inode_block.inodes[inodenum%16].mappings[1]);
		encrypt(mounts[mount_point].key,(char*)&inode_block,BLOCKSIZE);
		write_datablock(mount_point,1,(char*)&inode_block);
	}
...
int alloc_datablock(int mount_point){
	...
	*/
	...
	for(int i=3;i<MAX_BLOCKS;i++){
       if(my_device.block_bitmap[i]==(char)0){
		 //found one::
		 found=i;
		 my_device.block_bitmap[i]==(char)0;
		 my_device.used_blocks+=1;
		 break;
	   }
	}
	if(found==-1){
...
void free_datablock(int mount_point, int blocknum){
	...
	*/
	...
	read_superblock(mount_point,&my_device);
	my_device.block_bitmap[blocknum]=(char)0;
	my_device.used_blocks-=1;
